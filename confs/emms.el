...
(use-package emms
	...
	(defun my/emms-copy-marked-tracks (target-dir)
	"Copy all tracks that have been marked in the current buffer."
	(interactive "DTarget directory:")
	(emms-mark-do-with-marked-track
	 ;; Call this function on every marked line of track
		 (lambda nil
			 (let ((path-to-track
							 ;; Remove marker asterisk
							 (replace-regexp-in-string "^\*" "/"
																				 ;; Parse the next track
																					(buffer-substring (point)
																														(progn (end-of-line) (point))))))
				 ;; Copy track (overwrite is enabled)
				 (copy-file path-to-track target-dir t)
				 (message "copying tracks finished")))))

	(defun my/emms-act-on-tracks (fun)
		(if (string-match "\*Music" (buffer-name))
				(save-excursion
					(beginning-of-buffer)
					(while (= (forward-line 1) 0)
				 (funcall fun (emms-score-track-at-filename))))
			(message "Not an emms buffer")))

	(defun my/emms-mark-score ()
		"Mark tracks with score equal to use defined value (default) or greater and equal (positive prefix argument) or lesser and equal (negative prefix argument)")

	(defun my/emms-score-set-score-of-marked ()
		"set the score of marked tracks"
		(interactive)
		(my/emms-act-on-tracks message))
	...
	(
	 ...
	 ("<C-XF86AudioPlay>" . emms-play-playlist)
