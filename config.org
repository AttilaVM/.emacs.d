	...
	(save-excursion
		(let ((beg (point))
					(is-upcase (attila/up-case-p
											(get-byte (- (point) 1)))))
			(message (symbol-name is-upcase))
			(if is-upcase
					(downcase-region beg (search-backward " "))
				(upcase-region beg (search-backward " "))))))
...
#+begin_src emacs-lisp :tangle yes
(global-set-key (kbd "M-i") 'previous-history)  
(global-set-key (kbd "M-i") 'next-history-element)  
#+end_src
...
#+begin_src emacs-lisp :tangle yes
	...
	(require 'json-mode)
	(define-key json-mode-map (kbd "<insert> j v p") 'json-pretty-print-buffer)
	(define-key json-mode-map (kbd "<insert> j v o") 'json-pretty-print-buffer-ordered)
