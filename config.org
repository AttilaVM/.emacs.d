...
#+begin_src emacs-lisp :tangle yes
    ...
    (use-package a             :defer  :straight t)
...
**** Helm do ag search list

#+begin_src emacs-lisp :tangle yes
(setq attila/ag-search-alist
	  (a-list "linux-doc" "/home/attila/projects/c-examples/linux/Documentation/"
			  "linux-all" "/home/attila/projects/c-examples/linux/"
			  "org" "/home/attila/.emacs.d/straight/repos/org/doc/org-manual.org"))

(defun attila/doc-search ()
  (interactive)
  (let* ((search-target (helm :sources
			(helm-build-sync-source
				"kubernetes-actions"
			  :candidates (a-keys attila/ag-search-alist))))
	   (search-path (a-get attila/ag-search-alist search-target)))
  (cond
   ((f-directory-p search-path)
	(helm-do-ag search-path))
   ((f-file-p search-path)
	(helm-do-ag (f-dirname search-path) (f-filename search-path))))))

(global-set-key (kbd "<insert> h f") 'attila/doc-search)
#+end_src
...
(global-set-key (kbd "<insert> x n") 'revert-buffer)
...
(defun attila/helm-ibuffer-filter-group ()
  (interactive)
  (when (string-equal major-mode "ibuffer-mode")
	(let* ((candidates
			(-as-> ibuffer-saved-filter-groups X
				   (-map (lambda (x) (car x)) X)))
		   (selected-filter-group
			(helm :sources
				  ...
				  :buffer
				  "*helm-ibuffer-filter-groups*")))
	  (ibuffer-switch-to-saved-filter-groups selected-filter-group))))

(global-set-key (kbd "<insert> b i") 'ibuffer)
(define-key ibuffer-mode-map (kbd "<insert> j f") 'attila/helm-ibuffer-filter-group)
(define-key ibuffer-mode-map (kbd "c") 'ibuffer-diff-with-file)
(define-key ibuffer-mode-map (kbd "x") 'ibuffer-do-save)
...
#+begin_src emacs-lisp :tangle yes
	...
	(use-package flyspell
		...
		(setq attila/ispell-languages '("en_US" "hu_HU"))

		(defun attila/choose-flyspell-dictionary ()
		  (interactive)
		  (setq ispell-dictionary (helm :sources
				(helm-build-sync-source
					"ispell-languages"
				  :candidates attila/ispell-languages))))
...
**** Go style elisp extension fetcher

For packages which are not in melpa :(

#+begin_src emacs-lisp :tangle yes
(setq attila/elisp-path "~/.emacs.d/extensions")

(require 'magit)
(defun attila/clone-repo (url)
  "Go style repository cloning"
  (let* ((components
		(split-string url "/"))
	   (platform
		(nth 2 components))
	   ...
	   (repository
		(nth 4 components))
	   (import-path
		(f-join attila/elisp-path platform author repository)))
	(make-directory import-path t)
	(magit-run-git "clone" url import-path)
	(push import-path  load-path)
	import-path))
#+end_src
...
(when (f-dir-p "/usr/share/emacs/site-lisp/maxima/")
...
#+begin_src emacs-lisp :tangle yes
	...
	(use-package org
		...
		(defun attila/org-show-all-inline-images ()
		  (interactive)
		...
		(:map org-mode-map
					...
					("<insert> j v i" . attila/org-show-all-inline-images)
...
(add-hook
 ...
 (lambda ()
   ...
   (beginning-of-line)
   (let ((match (search-forward-regexp "\\.svg\\ ?\\]" nil t)))
	 (widen)
	 (when match
...
(setenv "PYTHONPATH"
		(mapconcat (lambda (item)
					 (if (stringp item)
						 item
					   (eval item)))
				   '((f-join (getenv "HOME") ".local/lib/python3.6/")
					 (getenv "PYTHONPATH"))
				   ":"))
#+end_src

#+RESULTS:
: /home/attila/.local/lib/python3.6/:/home/attila/.local/lib/python3.6/:
